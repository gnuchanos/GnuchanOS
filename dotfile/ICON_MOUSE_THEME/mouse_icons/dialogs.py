...
it is the ring and the bar, which is the shape that sign has had since long
before it was a cursor.
"""
...
from .cursor import FILL, Geometry, OUTLINE, SYMBOL
from .draw import arc, disc, polygon, ring
from .marks import paint, paint_badge, paint_symbol
...
def _question(canvas: Canvas, geometry: Geometry) -> None:
    ...
    arc(canvas, cx, cy - radius * 0.30, radius * 0.38,
        max(1.0, radius * 0.34), SYMBOL, math.pi * 0.66, math.pi * 1.32)
    half = max(0.5, radius * 0.17)
    polygon(canvas, [
    ...
    disc(canvas, cx, cy + radius * 0.74, max(0.8, half * 1.45), SYMBOL)
...
def draw_forbidden(canvas: Canvas, geometry: Geometry, frame: int,
                   frames: int) -> None:
    """The bar across a ring: the one state that is not about a pointer.

    The bar is drawn first and both shapes are filled in the same colour, so the
    pair reads as one sign rather than as a ring with something laid over it -
    which is the whole difference between a prohibition and a decorated circle.
    """
    centre = geometry.centre
    radius = palette.SPINNER_REACH * geometry.size
    width = palette.SPINNER_WIDTH * geometry.size
    paint(canvas, geometry, paths.turned(paths.SLASH, 0.125))
    ring(canvas, centre, centre, radius, width + 2.0 * geometry.outline,
         OUTLINE)
    ring(canvas, centre, centre, radius, width, FILL)
...
def draw_dnd_none(canvas: Canvas, geometry: Geometry, frame: int,
                  frames: int) -> None:
    """The pointer with a cross on the badge: nothing can be dropped here."""
    _pointer_and(canvas, geometry, _glyph(paths.turned(paths.CROSS, 0.125)))
