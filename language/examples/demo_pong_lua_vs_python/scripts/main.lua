...
local WIN_W = 800
local WIN_H = 600
local WORLD_W = 1600          -- iki pencere genisligi
local SCREEN_OFFSET = 0       -- Lua penceresi sol yariyi gosterir (dunya x 0..800)

local PADDLE_W = 12
local PADDLE_H = 90
local BALL_R = 8
local LU_PADDLE_X = 20                       -- Lua paddle (sol, dunya x)
local SPEED = 4.0
...
while not raylib.WindowShouldClose() do
    ...
    -- Lua AI: kendi paddle'ini (sol, local) topa gore oynat
    ...
    -- sol paddle carpismasi (dunya x)
    if bx - BALL_R <= LU_PADDLE_X + PADDLE_W and by >= lua_pad and by <= lua_pad + PADDLE_H and vx < 0 then
        vx = -vx
        bx = LU_PADDLE_X + PADDLE_W + BALL_R
    end

    -- skor: sol uc gecildi -> Python (sag) kazandi, ben (Lua) kaybettim
    if bx < 0 then
        lua_score = lua_score + 0  -- kendi skorunu tut (local); Python kendi tarafinda kazandi
        bx, by, vx, vy = 810, WIN_H / 2, 4.0, 2.0
    elseif bx > WORLD_W then
        lua_score = lua_score + 1  -- Python'un ucunu gectim, ben kazandim
    ...
    -- cizim (sol yari: dunya x 0..800 -> ekran x 0..800)
    raylib.BeginDrawing()
    raylib.ClearBackground(raylib.DARKPURPLE)

    -- orta sinir cizgisi (ekran x=800)
    local y = 0
    while y < WIN_H do
    ...
    -- Lua paddle (kendi, local): ekran x = 20
