...
static bool is_type_or_alias(GclParser *p) {
    if (is_type_token(p->current.kind)) {
        /* Multi-word type support: unsigned/long can be followed by another type */
        if (p->current.kind == TOK_KW_UNSIGNED || p->current.kind == TOK_KW_LONG || 
            p->current.kind == TOK_KW_SHORT) {
            return true;  /* Will be handled with the next token */
        }
        return true;
    }
    if (p->current.kind == TOK_IDENT) {
...
static GclAstNode *parse_primary_expr(GclParser *p) {
    ...
    if (t.kind == TOK_KW_SIZEOF) {
        /* sizeof(type_or_expr) — emit AST_SIZEOF_EXPR so IR gen can
         * resolve the byte size at compile time. str_value = type name. */
        parser_advance(p);
        GclAstNode *n = gcl_ast_new(p->arena, AST_SIZEOF_EXPR, line, col);
        if (parser_check(p, TOK_LPAREN)) {
            parser_advance(p);
            /* Collect everything until the matching ')' as a type name /
             * expression.  We only care about the raw token text for type
             * resolution; pointer stars are counted as pointer depth. */
            char type_buf[256] = {0};
            size_t tlen = 0;
            int ptr_depth = 0;
            int depth = 1; /* paren depth */
            while (!parser_check(p, TOK_EOF) && depth > 0) {
                if (parser_check(p, TOK_LPAREN)) { depth++; parser_advance(p); continue; }
                if (parser_check(p, TOK_RPAREN)) { depth--; if (depth == 0) break; parser_advance(p); continue; }
                if (parser_check(p, TOK_STAR)) { ptr_depth++; parser_advance(p); continue; }
                /* accumulate token text as type string */
                GclToken tt = p->current;
                size_t l = tt.length < 64 ? tt.length : 64;
                if (tlen + l + 1 < sizeof(type_buf)) {
                    if (tlen > 0) { type_buf[tlen++] = ' '; }
                    memcpy(type_buf + tlen, tt.start, l);
                    tlen += l;
                    type_buf[tlen] = '\0';
                }
                parser_advance(p);
            ...
            if (parser_check(p, TOK_RPAREN)) parser_advance(p);
            n->str_value  = gcl_intern(p->intern, type_buf, tlen);
            n->is_pointer = ptr_depth;
        }
        return n;
    }
...
static GclAstNode *parse_postfix_expr(GclParser *p) {
    ...
    for (;;) {
        if (parser_check(p, TOK_LPAREN)) {
            ...
            un->is_pointer = 1;  /* Mark as POST-increment/decrement */
            parser_advance(p);
            left = un;
        } else {
            break;
...
static GclAstNode *parse_unary_expr(GclParser *p) {
    ...
    if (parser_check(p, TOK_LPAREN)) {
        /* Lookahead: is this a type cast or just grouping? */
        size_t saved_pos = p->lex.pos;
        int saved_line = p->lex.line;
        int saved_col = p->lex.col;
        GclToken saved_token = p->current;
        
        parser_advance(p); /* skip '(' */
        
        /* Check if next token is a type */
        if (is_type_or_alias(p)) {
            /* Likely a cast - consume the type */
            parser_advance(p);
            ...
            if (parser_check(p, TOK_RPAREN)) {
                parser_advance(p); /* skip ')' */
                /* This is a cast - parse the operand */
                int line = saved_token.line, col = saved_token.col;
                GclAstNode *cast = gcl_ast_new(p->arena, AST_CAST_EXPR, line, col);
                cast->str_value = "cast";
                gcl_ast_add_child(cast, parse_unary_expr(p));
                return cast;
            }
        }
        
        /* Not a cast - restore and continue with normal parsing */
        p->lex.pos = saved_pos;
...
static GclAstNode *parse_assign_expr(GclParser *p) {
    ...
    if (parser_check(p, TOK_EQ) || parser_check(p, TOK_PLUS_EQ) ||
        ...
        /* Part 2/8: Handle struct/dict initialization: p = {...} */
        if (parser_check(p, TOK_LBRACE)) {
            gcl_ast_add_child(assign, parse_brace_initializer(p));
        } else {
 ...
 * Mixed → null. 
 * Part 2 extended: .field = identifier/expression values now supported. */
static GclAstNode *parse_brace_initializer(GclParser *p) {
    int line = p->current.line, col = p->current.col;
    parser_advance(p); /* skip '{' */
    char buf[2048];
    ...
    while (!parser_check(p, TOK_EOF) && !parser_check(p, TOK_RBRACE)) {
        /* Support nested braces for multi-dimensional arrays */
        if (parser_check(p, TOK_LBRACE)) {
            has_nested = 1;
            GclAstNode *nested = parse_brace_initializer(p);
            if (nested && nested->str_value) {
                const char *nested_str = nested->str_value;
                /* Remove quotes from nested string if present */
                const char *start = nested_str;
                const char *end = nested_str + strlen(nested_str);
                if (start[0] == '"' && end > start + 1 && end[-1] == '"') {
                    start++;
                    end--;
                }
                size_t nested_len = (size_t)(end - start);
                if (n + nested_len + 3 < sizeof(buf)) {
                    buf[n++] = '[';
                    memcpy(buf + n, start, nested_len);
                    n += nested_len;
                    buf[n++] = ']';
                }
            }
            ...
            if (parser_check(p, TOK_EQ) && fname) {
                ...
                /* Part 2: now accept any primary expression (literals, identifiers) */
                GclToken t = p->current;
    ...
    if (has_int || has_nested) {
        /* int array or nested array: "I;1;2;3" or "I;[;1;];[;2;]" */
        char out[1150];
        size_t m = 0;
        out[m++] = '"';
        out[m++] = 'I';
...
static GclAstNode *parse_var_decl(GclParser *p) {
    ...
    /* Handle multi-word types: unsigned/long/short can combine with multiple types */
    const char *first_word = type_name;
    GclTokenKind first_kind = p->current.kind;
    parser_advance(p);
    
    /* Collect all consecutive type keywords (e.g., unsigned long long) */
    char type_buf[256];
    snprintf(type_buf, sizeof type_buf, "%s", first_word);
    size_t tlen = strlen(type_buf);
    
    while ((first_kind == TOK_KW_UNSIGNED || first_kind == TOK_KW_LONG || first_kind == TOK_KW_SHORT) &&
           is_type_token(p->current.kind) && tlen < 240) {
        snprintf(type_buf + tlen, sizeof(type_buf) - tlen, " %s", tok_str(p, &p->current));
        tlen = strlen(type_buf);
        first_kind = p->current.kind;  /* Update for next iteration */
        parser_advance(p);
    }
...
static GclAstNode *parse_func_decl(GclParser *p) {
    ...
    const char *first_word = func->type_name;
    GclTokenKind first_kind = p->current.kind;
    parser_advance(p);
    
    /* Handle multi-word return types: unsigned long long, etc */
    char fbuf[256];
    snprintf(fbuf, sizeof fbuf, "%s", first_word);
    size_t flen = strlen(fbuf);
    
    while ((first_kind == TOK_KW_UNSIGNED || first_kind == TOK_KW_LONG || first_kind == TOK_KW_SHORT) &&
           is_type_token(p->current.kind) && flen < 240) {
        snprintf(fbuf + flen, sizeof(fbuf) - flen, " %s", tok_str(p, &p->current));
        flen = strlen(fbuf);
        first_kind = p->current.kind;
        parser_advance(p);
    }
    
    func->type_name = gcl_intern(p->intern, fbuf, strlen(fbuf));
    ...
    while (!parser_check(p, TOK_RPAREN) && !parser_check(p, TOK_EOF)) {
        if (is_type_or_alias(p) || parser_check(p, TOK_IDENT)) {
            ...
            /* Handle multi-word parameter types: unsigned long long, etc */
            const char *pword = param->type_name;
            GclTokenKind pkind = p->current.kind;
            parser_advance(p);
            
            char pbuf[256];
            snprintf(pbuf, sizeof pbuf, "%s", pword);
            size_t plen = strlen(pbuf);
            
            while ((pkind == TOK_KW_UNSIGNED || pkind == TOK_KW_LONG || pkind == TOK_KW_SHORT) &&
                   is_type_token(p->current.kind) && plen < 240) {
                snprintf(pbuf + plen, sizeof(pbuf) - plen, " %s", tok_str(p, &p->current));
                plen = strlen(pbuf);
                pkind = p->current.kind;
                parser_advance(p);
            }
            
            param->type_name = gcl_intern(p->intern, pbuf, strlen(pbuf));
...
static bool looks_like_decl(GclParser *p) {
    ...
    /* Handle multi-word types: skip all consecutive type keywords */
    GclTokenKind first_kind = saved_tok.kind;
    while ((first_kind == TOK_KW_UNSIGNED || first_kind == TOK_KW_LONG || 
            first_kind == TOK_KW_SHORT) && is_type_token(p->current.kind)) {
        first_kind = p->current.kind;
        parser_advance(p);
    }
