...
/* Asynchronous external process execution abstraction.
   When the "Run" command is executed synchronously via popen/fgets, the IDE locks up and
   the child process remains open when the IDE closes. This structure starts the process asynchronously,
   collects output every frame, and terminates the process when the IDE closes.
...
/* GNU FreeFont — embedded byte array for FreeMono.ttf (generated by makefile.py embed_font). */
...
/* FreeFont wrapper — renders DrawText/MeasureText through our custom font. */
void gcl_draw_text(const char *text, int posX, int posY, int fontSize, Color color);
void gcl_draw_text_f(const char *text, float posX, int posY, int fontSize, Color color); /* float position — prevents truncation drift in syntax-highlight chunks */
int gcl_measure_text(const char *text, int fontSize);
float gcl_measure_text_f(const char *text, int fontSize); /* real float width — prevents cumulative drift in chunk rendering */

/* TAB-aware rendering/measurement. '\t' is used as indentation in Python files;
   if rendering and measurement do not share the same tab stops, the cursor drifts away from the text.
   'start_col' is the cell (column) index where the string begins. */
...
/* Replace raylib's DrawText/MeasureText functions with our wrapper. */
#undef DrawText
#undef MeasureText
...
/* Number of rendered frames to wait before the GCL_IDE_SHOT capture fires
   (see the capture hook in ide_main.c). A few frames are needed so that the
   placeholder textures, the font and the first layout are all in place. */
#define SHOT_AFTER_FRAMES 20
...
typedef struct Editor {
    ...
    int split_left_tab;
    int split_right_tab;
    float split_left_tab_scroll;
    float split_right_tab_scroll;
    GclIdeBuffer *split_right_tabs;
    int split_right_tab_count;
    ...
    /* split editor panes (horizontal side-by-side) */
    int split_enabled;
    float split_ratio;
    int split_dragging;

    /* NOTE: selection anchor (sel_anchor) is no longer stored here; it lives in
       GclIdeBuffer (gcl_ide.h). This way each tab owns its selection; undo/redo
       and tab switching do not corrupt the selection state (todo #6). */
    ...
    /* Last cursor position, PER PANE: cursor-follow (keeping the caret visible) only
       runs when the caret of THAT pane actually changes. The mouse wheel does not
       move the caret, so manual scrolling is preserved. A single shared field used
       to break this in split view: moving the caret in one pane made the other pane
       believe its caret had changed too, so the other pane's manual scroll was reset
       (todo bug #4/#17). Index: 0 = left/single pane, 1 = right. */
    size_t last_cursor[2];

    /* output panel */
    char output[16384];
    size_t output_len;
    int output_visible;
    int output_scroll;   /* output panel vertical scroll (line offset) */
    int running;
    ...
    /* typewriter pitch (ridiculous_coding pitch_increase) */
    float sound_pitch;      /* instantaneous pitch to play */
    float pitch_increase;   /* increases with each key, then fades over time */

    Sound click_snd;
    Sound enter_snd;   /* Enter: strong low "explosion" */
    Sound delete_snd;  /* Delete/Backspace: different "explosion" */
...
/* Split view OFF: move the right pane's tabs back into the main tab list so the
   file being edited in that pane stays open in single view (todo bug #23). */
int tab_fold_split_into_main(Editor *ed);

/* ---- pane state (ide_fs_tree.c) ------------------------------------------
   Pure Editor state, no raylib drawing: kept next to the tab functions and out of
   ide_main.c so the split/merge transitions can be covered by a headless test
   (todo bug #1-#7, #23). Pane 0 = the main tab list (ed->tabs / active_tab);
   pane 1 = the right pane's own list (split_right_tabs / split_right_tab). */
...
/* Focus a pane without touching the OTHER pane's selected tab. */
void editor_set_split_focus(Editor *ed, int pane);
/* Repair out-of-range pane indices; keep an empty pane focusable. */
...
/* Enter: new line + automatic block indentation (Python/GCL). */
void editor_insert_newline_autoindent(Editor *ed);

/* GCL workspace-aware symbol scanner (gcl_lsp_scan.c) */
...
/* Arrow key without Shift while text is selected: collapse the selection to the nearest edge (VSCode style).
   to_start=1 → left/top edge, to_start=0 → right/bottom edge. */
...
/* New Project panel (ide_new_project.c) */
...
/* Settings/About modal drawings — gcl_settings_panel.c (independent module) */

/* gcBundle build — the IDE performs the build internally (no external process is invoked).
   build_project used to call gcl.exe -build; in the IDE GCL_EXE_PATH/argv differ,
   causing gcb_build_project_runtime to fail. For that reason, the IDE calls these
   functions directly. */
...
/* gcl_os.c — dizin oluştur / temizleme (IDE build'inde out_dir için gerekli) */
